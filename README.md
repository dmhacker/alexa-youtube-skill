...
11. For the following step, you will need to have [the NodeJS runtime and npm package manager](https://nodejs.org/en/download/) installed. You can download them here. 
12. Change directory into __the js/ folder__ inside the project. Now, in the command line, type: "npm install". Wait for it to finish. This command will install all the required project dependencies into js/node_modules.
13. Change directory out of js/ with "cd ../" and run the file __zip.py__ in the command line using Python. The file will generate a zip file called __alexa-youtube-skill.zip__. 
14. Now, go back to the Lambda function you just saved. Under "Code entry type," select "Upload a ZIP file." Then, upload alexa-youtube-skill.zip under "Function Package." 
15. You will now need to enter 2 environment variables. These are:
...
16. The last step is linking your Lambda function to your Alexa skill. Go back to Alexa under Amazon Developer and find your skill. In the __Configuration__ page, put the Lambda ARN name in the blank spot that you left earlier.
17. Go to the __Test__ page and set Enabled to true. The skill will now work exclusively on your devices.
