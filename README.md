...
4. Once the skill has the video ID, it sends that a custom Heroku server that I built. The Heroku server takes the video ID, downloads the audio, and puts it into an S3 bucket. 
    ...
    * Additionally, the video is downloaded first on the server as an audio file, and then that audio file is uploaded to the Heroku server. This whole process is asynchronous. This means that when the server is pinged, the process begins but the request is returned immediately even before the audio file has finished uploading. The skill then has to have a way of detecting when the audio file is uploaded because the server will not notify the skill unless it is pinged. I bypassed this issue by having the skill ping the server every 3 seconds, checking to see if the server has uploaded the file. When the server has finished the upload, it will notify the skill when it is pinged, and the skill will halt.
